...
The result of the build is placed in the `.output/site` directory. Inside it,
...
- `static` contains other static files necessary for running the app: JavaScript, CSS, image and JSON files.

The `static` directory is designed to be served by Django or NGINX directly under `yoursite.example.com/static/`.
It should be included in the `STATICFILES_DIRS` in Django settings.
Please note that the files from `.output/site/static` are not the only files that need to be served under
...
directories specified in `STATICFILES_DIRS`, including the `.output/site/static` folder generated by Nuxt 4
...
Django will serve them directly from the `.output/site/html` directory (specified in the custom `GUTENBERG_SPA_HTML_DIR`
Django setting).
...
and changes the output directory for HTML files to `.output/site/html`.
