...
class TestFinalPageProcessor:
    ...
    def test_init_invalid_n(self, mock_page_sizes, tmp_path):
    ...
    def test_create_final_pages_success(self, mock_file, mock_writer_cls, mock_reader_cls, mock_page_sizes, tmp_path):
    ...
    @patch("printing.processing.final_pages.PdfWriter")
    @patch("builtins.open", new_callable=mock_open)
    def test_create_final_pages_fit_to_page_disabled(self, mock_file, mock_writer_cls, mock_reader_cls, mock_page_sizes, tmp_path):
        mock_reader = mock_reader_cls.return_value
        mock_writer = mock_writer_cls.return_value
        ...
        fp = FinalPageProcessor(str(tmp_path), 1, mock_page_sizes, PageOrientation.PORTRAIT, fit_to_page=False)
        fp.create_final_pages("in.pdf", "1")

        mock_page.scale_by.assert_not_called()

    def test_create_final_pages_no_pages(self, mock_page_sizes, tmp_path):
