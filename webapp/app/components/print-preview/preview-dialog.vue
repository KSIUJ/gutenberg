<template>
  ...
  >
    ...
    <!-- Main viewer -->
    <div class="flex min-h-0 grow items-center justify-center bg-surface-50 border-t border-b border-surface gap-2 p-3 sm:gap-4 sm:p-6">
    ...
    <div
      v-if="units.length > 1"
      class="flex shrink-0 items-center gap-1 px-1 py-2 border-surface border-b"
    >
      <p-button
...
onKeyStroke('ArrowLeft', (event) => {
  if (!props.preview.visible) return;
