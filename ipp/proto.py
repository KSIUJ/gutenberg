...
from ipp.fields import CharsetField, NaturalLangField, TAG_STRUCT, IppFieldsStruct, \
    ParserState, OneSetField, KeywordField, NameWLField
...
class AttributeGroup(IppFieldsStruct):
    ...
    def read_from(cls, readable, state: ParserState):
        ...
        while True:
            ...
            if state.current_name:
                field = fields.get(state.current_name)
                if not field:
                    continue
                read_fields[state.current_name] = field.read(readable, state)
            else:
                pass
    ...
    def _filter_fields(cls, requested_attrs: List[str]):
        fields = cls._get_proto_fields()
        if requested_attrs is None or 'all' in requested_attrs or cls._filter in requested_attrs:
            return fields
        return {k: v for k, v in fields.items() if k.replace('_', '-') in requested_attrs}
...
class IppRequest(IppMessage):
    def __init__(self, buffer, version: Tuple[int, int] = IppMessage.IPP2_0, opid_or_status: int = StatusCodeEnum.ok,
                 request_id: int = 0):
        super().__init__(version, opid_or_status, request_id)
        self._parser_state = ParserState()
        self._parser_state.read_field_header(buffer)
        self._buffer = buffer
    ...
    def read_group(self, attribute_group_type: Type[AttributeGroup]) -> Any:
        ...
        return attribute_group_type.read_from(self._buffer, self._parser_state)
...
def response_for(request: IppRequest, operation_groups: List[AttributeGroup], status: int = StatusCodeEnum.ok):
    return IppResponse(request.version, status, request.request_id, operation_groups)
